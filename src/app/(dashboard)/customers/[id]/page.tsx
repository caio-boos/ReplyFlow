        ...
        new Map(
          customer.emails_list
            .map((email) => {
              ...
              return [
                id,
                { id, label: email.accountEmail ?? email.accountId ?? id },
              ] as const;
            })
            .filter(
              (entry): entry is readonly [string, AccountViewOption] =>
                entry !== null,
            ),
        ).values(),
      )
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div className="bg-gray-900/60 border border-white/6 rounded-xl overflow-hidden">
        <div className="flex items-start gap-4 px-5 py-5">
          <div
            className={`w-12 h-12 rounded-xl bg-linear-to-br ${gradient} flex items-center justify-center shrink-0 text-white text-base font-semibold`}
          >
            {initials}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {customer.emails.map((e) => (
                <span
                  key={e}
                  className="text-xs font-mono text-gray-400 bg-gray-800/60 border border-white/5 px-2 py-0.5 rounded-md"
                >
                  {e}
                </span>
              ))}
        ...
        {customer.orderNumbers.length > 0 && (
          <div className="px-5 py-3 border-t border-white/5 bg-black/10 flex items-center gap-3 flex-wrap">
            ...
            {customer.orderNumbers.map((o) => (
              <span
                key={o}
                className="text-xs px-2 py-0.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-md font-mono"
              >
                #{o}
              </span>
            ))}
          ...
          <div className="px-5 py-4 border-t border-white/5">
            ...
            <div className="space-y-2">
              {customer.suggestedLinks.map((s, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2 bg-amber-500/5 border border-amber-500/15 rounded-lg px-3 py-2.5 text-xs text-amber-300/80"
                >
                  <span className="mt-0.5 shrink-0 w-4 h-4 rounded-full bg-amber-500/15 flex items-center justify-center text-amber-400 font-medium">
                    {i + 1}
                  </span>
                  {s.reason}
                </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-gray-200">Conversa</h2>
          ...
          <div className="space-y-4">
            {visibleEmails.map((email) => {
              ...
              return (
                <div
                  key={email.id}
                  className="bg-gray-900/50 border border-white/6 rounded-xl overflow-hidden"
                >
                  {/* Email header */}
                  <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-white/5">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span
                        className={`flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-md border ${sc.bg} ${sc.text} ${sc.border}`}
                      >
                        <span
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {email.bodyText && (
                      <div className="flex gap-3">
                        <div
                          className={`shrink-0 w-7 h-7 rounded-lg bg-linear-to-br ${gradient} flex items-center justify-center text-xs text-white font-semibold`}
                        >
                          {initials}
                        </div>
