...
export async function POST(req: NextRequest) {
  ...
  if (productLink && !isSafeUrl(productLink))
    return NextResponse.json(
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: `Não foi possível acessar a URL: ${err instanceof Error ? err.message : "erro"}`,
      },
      { status: 400 },
    );
  }
  ...
  $("script").remove();
  $(
    'link[rel="preload"], link[rel="preconnect"], link[rel="prefetch"], link[rel="dns-prefetch"]',
  ).remove();
  $(
    'meta[name^="shopify"], meta[name="shopify-y"], meta[name="shopify-s"]',
  ).remove();
  ...
  $("[style]").each((_, el) => {
    ...
    const updated = style.replace(
      ...
      () => `background-image: url([IMGBG${++imgBgCounter}])`,
    );
  ...
  function walkNode(node: unknown) {
    ...
    if (htmlNode.type === "text") {
      ...
      const raw: string = htmlNode.data ?? "";
      ...
      htmlNode.data =
        raw.slice(0, start) + `%%${key}%%` + raw.slice(start + trimmed.length);
    } else if (htmlNode.type === "tag" || htmlNode.type === "root") {
      const tagName = (htmlNode.name ?? "").toLowerCase();
      if (SKIP_TAGS.has(tagName)) return;
      for (const child of htmlNode.children ?? []) walkNode(child);
    }
  }
...
- Return ONLY a valid JSON object with the same keys and translated values.`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: `Erro na IA: ${err instanceof Error ? err.message : "erro"}` },
      { status: 500 },
    );
  }
}
