...
const PROVIDER_DEFAULTS: Record<string, { imapHost: string; imapPort: number; smtpHost: string; smtpPort: number }> = {
  godaddy: { imapHost: "imap.secureserver.net", imapPort: 993, smtpHost: "smtpout.secureserver.net", smtpPort: 465 },
  hostinger: { imapHost: "imap.hostinger.com", imapPort: 993, smtpHost: "smtp.hostinger.com", smtpPort: 465 },
...
export async function GET() {
  ...
  const db = getAdminDb();
  const snap = await db.collection("accounts").orderBy("createdAt", "asc").get();

  const accounts = snap.docs.map((d) => {
    const data = d.data();
    // Never return encrypted password to client
    const { encryptedPassword: _omit, ...safe } = data;
    return { id: d.id, ...safe };
  });
